import { cleanContent, getContentStats } from './api';
...
export function configure(adminEmail, provider) {
...
async function processQueue() {
  ...
  while (state.queue.length > 0) {
    ...
    try {
      const { data } = await cleanContent(email, state.adminEmail, state.provider);
      await new Promise((resolve) => setTimeout(resolve, 3000));
      let stats = null;
      try {
        const { data: s } = await getContentStats(email, state.adminEmail, state.provider);
        stats = s;
      } catch { /* ignore */ }
