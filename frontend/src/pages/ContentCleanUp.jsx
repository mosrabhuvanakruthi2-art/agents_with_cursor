...
import {
  getSourceUsers, getContentStats,
  cleanContentFiles, cleanContentFolders,
} from '../services/api';
import {
  configure, startClean, startCleanAll, subscribe,
  getActiveCleans, getAllResults, clearResults,
} from '../services/cleanContentManager';
...
export default function ContentCleanUpPage({ embedded = false }) {
  ...
  const [fetched, setFetched] = usePersistedState('clean-content-fetched', false);
    ...
    try {
      ...
      for (let i = 0; i < updatedUsers.length; i += batchSize) {
        ...
        const results = await Promise.allSettled(
          batch.map((u) => getContentStats(u.email, adminEmail, provider))
        );
  ...
  const handleCleanUser = useCallback((email) => {
    if (!window.confirm('Delete ALL files and folders from:\n\n' + email + '\n\nThis cannot be undone. Continue?')) return;
    setCleanResults((prev) => { const next = { ...prev }; delete next[email]; return next; });
    startClean(email);
  }, []);

  const handleCleanAll = useCallback(() => {
    const toClean = users.filter((u) => u.stats && !u.stats.error && ((u.stats.fileCount ?? 0) > 0 || (u.stats.folderCount ?? 0) > 0));
    if (toClean.length === 0) return;
    if (!window.confirm('Delete ALL files and folders from ' + toClean.length + ' user(s):\n\n' + toClean.map((u) => u.email).join('\n') + '\n\nThis cannot be undone. Continue?')) return;
  ...
  const refreshUserStats = useCallback(async (email) => {
    try {
      const { data } = await getContentStats(email, adminEmail, provider);
      setUsers((prev) => prev.map((u) => u.email === email ? { ...u, stats: data } : u));
    } catch { /* ignore */ }
  }, [adminEmail, provider, setUsers]);

  const handlePartialClean = useCallback(async (email, field) => {
    setConfirmTarget(null);
    setPartialState((p) => ({ ...p, [email]: { ...(p[email] || {}), [field]: 'running' } }));
    try {
      const apiFn = field === 'files' ? cleanContentFiles : cleanContentFolders;
      const { data } = await apiFn(email, adminEmail, provider);
      const deleted = data.deleted || {};
      const count = field === 'files' ? (deleted.deletedFiles ?? 0) : (deleted.deletedFolders ?? 0);
      if (count === 0 && (deleted.errors?.length ?? 0) > 0) {
        showToast(`Delete failed: ${deleted.errors[0]}`, 'error');
      } else {
        showToast(`Deleted ${count} ${field} from ${email}`);
      }
  ...
  }, [adminEmail, provider, setUsers, showToast, refreshUserStats]);
  ...
  const totalFiles = users.reduce((sum, u) => sum + (u.stats?.fileCount || 0), 0);
  const totalFolders = users.reduce((sum, u) => sum + (u.stats?.folderCount || 0), 0);
  const usersWithData = users.filter((u) => u.stats && !u.stats.error && ((u.stats.fileCount ?? 0) > 0 || (u.stats.folderCount ?? 0) > 0));
        ...
        <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-4 flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-medium text-yellow-800">
              Cleaning in progress ({activeCleans.size} user{activeCleans.size > 1 ? 's' : ''})
            </p>
      ...
      {fetched && (
        <>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <StatCard label="Users" value={users.length} color="indigo" />
            <StatCard label="Total Files" value={statsLoading ? '...' : totalFiles} color="blue" />
            <StatCard label="Total Folders" value={statsLoading ? '...' : totalFolders} color="purple" />
          </div>
          ...
          <div className="flex justify-end gap-3">
            ...
            {usersWithData.length > 0 && (
              ...
              >
                <TrashIcon />
                Clean All {usersWithData.length} User{usersWithData.length > 1 ? 's' : ''}
              </button>
            )}
          </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-200">
                    ...
                    <th className="px-5 py-3 text-right text-xs font-medium text-gray-500 uppercase">Files</th>
                    <th className="px-5 py-3 text-right text-xs font-medium text-gray-500 uppercase">Folders</th>
                ...
                <tbody className="divide-y divide-gray-100">
                  {users.map((user) => {
                    ...
                    const isClean = s && !hasError && (s.fileCount ?? 0) === 0 && (s.folderCount ?? 0) === 0;
                    const result = cleanResults[user.email];
                    const isCleaning = activeCleans.has(user.email);

                    return (
                      <tr key={user.email} className={'hover:bg-gray-50' + (isClean ? ' bg-green-50/50' : '') + (isCleaning ? ' bg-yellow-50/50' : '') + (hasError ? ' bg-orange-50/30' : '')}>
                        <td className="px-5 py-3">
                          ...
                          {s?.error && typeof s.error === 'string' && (
                            <p className="text-xs text-orange-500 mt-0.5">⚠ {s.error}</p>
                          )}
                        </td>
                        <PartialCell
                          stats={s}
                          ...
                          field="files"
                          email={user.email}
                          partialField={partialState[user.email]?.files}
                          isConfirming={confirmTarget?.email === user.email && confirmTarget?.field === 'files'}
                          ...
                          <div className="flex flex-col items-end gap-1">
                            {result && !result.error && result.deleted && (
                              <span className="text-xs text-green-600 font-medium">
                                Deleted {result.deleted.deletedFiles ?? 0} files, {result.deleted.deletedFolders ?? 0} folders
                              </span>
                            )}
                            {result?.error && (
                            ...
                            {isCleaning ? (
                              ...
                              <button
                                ...
                                disabled={!s || hasError || isCleaning}
...
function PartialCell({ stats, count, hasError, field, email, partialField, isConfirming, onRequestConfirm, onConfirm, onCancelConfirm, isRunning }) {
  ...
  return (
    <td className="px-5 py-3 text-right">
      {isZero || hasError ? (
        <span className={hasError ? 'text-orange-400' : 'text-green-600'}>
