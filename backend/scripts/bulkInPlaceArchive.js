/**
 * bulkInPlaceArchive.js
 *
 * Creates bulk emails in all subfolders of InPlace_Archive_Test for ron@qatestagent.com:
 *   - l1 : 5,000 emails
 *   - l5 : 5,000 emails
 *   - all other subfolders : 500 emails each
 *
 * All emails use old timestamps (2019-2024).
 * Covers: attachments, HTML/plain bodies, emojis, links, threads (5-10 levels),
 *         flagged/important, categories, read/unread, CC/BCC/multi-TO.
 *
 * Usage: cd backend && node scripts/bulkInPlaceArchive.js
 */
...
// ─── Timestamp helpers (all old: 2019-2024) ───────────────────────────────────
...
function buildZip(files) {
  ...
  for(const{name,data}of files){
    ...
    off+=lh.length+data.length; parts.push(lh,data);
  }
  const cd=Buffer.concat(central),cdOff=off;
  const eocd=Buffer.concat([Buffer.from([0x50,0x4B,0x05,0x06]),u16(0),u16(0),u16(central.length),u16(central.length),u32(cd.length),u32(cdOff),u16(0)]);
  return Buffer.concat([...parts,cd,eocd]);
}

// ─── Attachment blob builders ─────────────────────────────────────────────────
function buildRawPdf(title) {
  const lines = [`QA Archive Test — ${title}`, 'InPlace_Archive_Test folder seeding', 'All content is synthetic test data.'];
  const ops = ['BT','/F1 16 Tf','72 720 Td'];
  lines.forEach((l,i)=>{ ops.push(`0 ${i===0?0:-24} Td`); ops.push(`(${l.replace(/[()\\]/g,'\\$&')}) Tj`); });
  ops.push('ET');
  const s=ops.join('\n')+'\n';
...
function buildDocx(title) {
  const contentXml = `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"><w:body><w:p><w:r><w:t>${title}</w:t></w:r></w:p><w:p><w:r><w:t>InPlace_Archive_Test — QA bulk data seeding for migration testing.</w:t></w:r></w:p><w:p><w:r><w:t>All content is synthetic. Folder: ${PARENT_FOLDER}</w:t></w:r></w:p></w:body></w:document>`;
  ...
  return buildZip([
    ...
    {name:'_rels/.rels',data:Buffer.from(`<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Relationships xmlns="${NS}package/2006/relationships"><Relationship Id="rId1" Type="${NS}officeDocument/2006/relationships/officeDocument" Target="word/document.xml"/></Relationships>`,'utf8')},
    {name:'word/document.xml',data:Buffer.from(contentXml,'utf8')},
  ]);
}
function buildXlsx() {
  const wb=XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb,XLSX.utils.aoa_to_sheet([['User','Folder','Messages','Status'],['Ron','InPlace_Archive_Test/l1',5000,'OK'],['Ron','InPlace_Archive_Test/l5',5000,'OK'],['Ron','InPlace_Archive_Test/other',500,'OK']]),'Archive Data');
...
// ─── Pre-build attachment blobs ───────────────────────────────────────────────
console.log('Building attachment blobs…');
const BLOBS = {
  pdf:  b64(buildRawPdf('Archive Test Report')),
  docx: b64(buildDocx('Archive Test Document')),
  xlsx: buildXlsx(),
  pptx: buildPptx('Archive Test Deck'),
  zip:  b64(buildZip([
    { name: 'archive-log.txt',  data: Buffer.from('InPlace_Archive_Test seeding log\nStep 1: folder discovery\nStep 2: message creation\nStep 3: validation\n', 'utf8') },
    { name: 'user-map.csv',     data: Buffer.from('Source,Dest\nron@qatestagent.com,ron@migrationn.com\ngranger@qatestagent.com,granger@migrationn.com\n', 'utf8') },
  ])),
  png:  PNG_B64,
  txt:  Buffer.from('InPlace_Archive_Test bulk seeding.\nLine 2: migration validation.\nLine 3: archive QA.\nLine 4: verify text content preserved.\n').toString('base64'),
  csv:  Buffer.from('Name,Email,Folder,Status\nRon,ron@qatestagent.com,l1,Active\nGranger,Granger@qatestagent.com,l5,Active\nAlex,Alex@qatestagent.com,l2,Active\n').toString('base64'),
};
...
const EMOJIS    = ['🚀','📧','✅','❌','⚠️','🔔','📊','📈','💼','🏆','🎯','🔥','💡','📌','⏰','🔄','🛡️','📋','🗓️','💬'];
const FLAGS_TXT = ['[ACTION REQUIRED]','[URGENT]','[FYI]','[EXTERNAL]','[REMINDER]','[FOLLOW UP]','[IMPORTANT]','[CONFIDENTIAL]','[HIGH PRIORITY]','[REVIEW NEEDED]'];
...
const SUBJECT_TEMPLATES = [
  ...
  (i)=>`Archive: ${pick(PROJECTS,i)} folder ${PARENT_FOLDER} — ${pick(QUARTERS,i)} ${pick(YEARS,i)}`,
  ...
  (i)=>`Cloud archive: ${pick(PROJECTS,i)} — ${pick(TOPICS,i)} review`,
  ...
  (i)=>`Archive validation: ${PARENT_FOLDER} — ${pick(TOPICS,i)} ${pick(YEARS,i)}`,
  ...
  // Multilingual / Unicode subjects
  (i)=>`Migración de correo — ${pick(PROJECTS,i)} ${pick(YEARS,i)}`,
  (i)=>`アーカイブ通知 — ${pick(DEPTS,i)} ${pick(QUARTERS,i)}`,
  (i)=>`Бэкап завершён: ${pick(SYSTEMS,i)} — ${pick(YEARS,i)}`,
  (i)=>`Archivage terminé — ${pick(PROJECTS,i)} ${pick(YEARS,i)}`,
  (i)=>`مراجعة الأرشيف — ${pick(TOPICS,i)}`,
];
...
const BCC_PATTERNS = [
  undefined, [ea(U.bt3)], undefined, [ea(U.bt1)], undefined,
  [ea(U.granger)], undefined, [ea(U.bt2)], undefined, undefined,
];
const TO_PATTERNS = [
  [ea(U.ron)],
  [ea(U.ron), ea(U.alex)],
...
function htmlBody(i, subject) {
  const t = i % 12;
  if (t === 0) return `<html><body style="font-family:Calibri,sans-serif;font-size:14px"><p>Hi ${pick(Object.values(U),i).name},</p><p>Please review the details below regarding <strong>${subject}</strong>.</p><ul><li>Item 1: Pre-scan complete ✅</li><li>Item 2: Migration in progress ⏳</li><li>Item 3: Validation pending 🔍</li></ul><p>Archive folder: <code style="background:#f4f4f4;padding:2px 6px">${PARENT_FOLDER}</code></p><p>References: ${pickLinks(i)}</p><p>— ${pick(SENDERS,i).name}</p></body></html>`;
  if (t === 1) return `<html><body style="font-family:Calibri,sans-serif;font-size:14px"><h2 style="color:#0078D4">${subject}</h2><p>Summary table:</p><table style="border-collapse:collapse;font-size:13px"><tr style="background:#dce6f1"><th style="border:1px solid #bbb;padding:4px 10px">Metric</th><th style="border:1px solid #bbb;padding:4px 10px">Value</th><th style="border:1px solid #bbb;padding:4px 10px">Status</th></tr><tr><td style="border:1px solid #bbb;padding:4px 10px">Messages</td><td style="border:1px solid #bbb;padding:4px 10px">${(i%500)+50}</td><td style="border:1px solid #bbb;padding:4px 10px">✅ OK</td></tr><tr><td style="border:1px solid #bbb;padding:4px 10px">Folders</td><td style="border:1px solid #bbb;padding:4px 10px">${(i%20)+3}</td><td style="border:1px solid #bbb;padding:4px 10px">✅ OK</td></tr><tr><td style="border:1px solid #bbb;padding:4px 10px">Errors</td><td style="border:1px solid #bbb;padding:4px 10px">0</td><td style="border:1px solid #bbb;padding:4px 10px">✅ OK</td></tr></table><p>More info: ${pickLinks(i,3)}</p></body></html>`;
  if (t === 2) return `<html><body style="font-family:Calibri,sans-serif;font-size:14px"><p>🚀 <strong>Update on ${subject}</strong></p><blockquote style="border-left:4px solid #0078D4;padding-left:12px;color:#333">Previously: migration was scheduled for ${pick(MONTHS,i)} ${pick(YEARS,i)}.<br/>Now: ahead of schedule — completed ${(i%30)+1} days early! 🎉</blockquote><p>Action items for <strong>${pick(DEPTS,i)} team</strong>:</p><ol><li>Verify mailbox access</li><li>Run validation agent</li><li>Sign off in JIRA</li></ol><p>Dashboard: ${pickLinks(i)}</p></body></html>`;
  if (t === 3) return `<html><body style="font-family:Calibri,sans-serif;font-size:14px"><p>⚠️ <span style="color:#cc0000"><strong>Alert: ${subject}</strong></span></p><p>The following issue was detected on <code style="background:#f4f4f4;padding:2px 6px">${pick(SYSTEMS,i)}</code>:</p><p style="background:#fff3cd;border:1px solid #ffc107;padding:8px 12px;border-radius:4px">Error: ${pick(TOPICS,i)} process failed at step ${(i%5)+1}. Retry count: ${(i%3)+1}. Status: DEGRADED.</p><p>Please investigate. Escalation contact: <a href="mailto:${pick(SENDERS,i+1).address}">${pick(SENDERS,i+1).name}</a></p><p>Runbook: ${pickLinks(i)}</p></body></html>`;
  if (t === 4) return `<html><body style="font-family:Calibri,sans-serif;font-size:14px"><p>Hi all 👋</p><p>Here's the ${pick(QUARTERS,i)} ${pick(YEARS,i)} recap for <strong>${pick(DEPTS,i)}</strong>:</p><ul><li>✅ ${pick(TOPICS,i)} completed on schedule</li><li>📊 ${(i%100)+50} items processed, ${(i%5)} errors</li><li>🏆 Top performer: <strong>${pick(SENDERS,i).name}</strong></li><li>🔔 Next milestone: ${pick(TOPICS,i+1)} by end of ${pick(MONTHS,i+1)}</li></ul><p>Full report: ${pickLinks(i)}</p></body></html>`;
  if (t === 5) return `<html><body style="font-family:Calibri,sans-serif;font-size:14px"><p>Following up on our earlier discussion about <strong>${subject}</strong>.</p><p>Key decisions made:</p><table style="border-collapse:collapse;font-size:13px"><tr style="background:#e2efda"><th style="border:1px solid #bbb;padding:4px 10px">Decision</th><th style="border:1px solid #bbb;padding:4px 10px">Owner</th><th style="border:1px solid #bbb;padding:4px 10px">Due</th></tr><tr><td style="border:1px solid #bbb;padding:4px 10px">${pick(TOPICS,i)} approach</td><td style="border:1px solid #bbb;padding:4px 10px">${pick(SENDERS,i).name}</td><td style="border:1px solid #bbb;padding:4px 10px">${pick(MONTHS,i+1)} 15</td></tr><tr><td style="border:1px solid #bbb;padding:4px 10px">Vendor selection</td><td style="border:1px solid #bbb;padding:4px 10px">${pick(SENDERS,i+1).name}</td><td style="border:1px solid #bbb;padding:4px 10px">${pick(MONTHS,i+2)} 1</td></tr></table><p>More: ${pickLinks(i)}</p></body></html>`;
  if (t === 6) return `<html><body style="font-family:Calibri,sans-serif;font-size:14px"><p>${pick(EMOJIS,i)} <strong>${subject}</strong></p><p style="background:#d4edda;border:1px solid #c3e6cb;padding:8px 12px;border-radius:4px">✅ <strong>COMPLETED SUCCESSFULLY</strong> — All ${(i%200)+50} items processed, 0 errors.</p><p>Validation results:</p><ul><li>Folder count: <strong>${(i%20)+5}/${(i%20)+5}</strong> ✅</li><li>Message count: <strong>${(i%500)+100}/${(i%500)+100}</strong> ✅</li><li>Attachment integrity: <strong>PASS</strong> ✅</li></ul><p>Signed off by: <strong>${pick(SENDERS,i).name}</strong> | Platform: ${pickLinks(i,1)}</p></body></html>`;
  if (t === 7) return `<html><body style="font-family:Calibri,sans-serif;font-size:14px"><p>Dear ${pick(Object.values(U),i).name},</p><p>This is a <em>formal notification</em> regarding <strong>${subject}</strong>.</p><p style="background:#f8d7da;border:1px solid #f5c6cb;padding:8px 12px;border-radius:4px">⚠️ Action required by <strong>${pick(MONTHS,i)} ${(i%28)+1}, ${pick(YEARS,i)}</strong>. Non-compliance may result in access restriction.</p><p>Contact <a href="mailto:${pick(SENDERS,i).address}">${pick(SENDERS,i).name}</a> for questions.</p><p>Policy reference: ${pickLinks(i)}</p></body></html>`;
  if (t === 8) return `<html><body style="font-family:Calibri,sans-serif;font-size:14px"><p>Hi team 😊</p><p>Quick update on <strong>${subject}</strong> — everything is on track! 🎯</p><ul><li>📅 Timeline: on schedule ✅</li><li>💰 Budget: within limits ✅</li><li>👥 Team: ${(i%10)+3} members active ✅</li><li>🔧 Technical: ${(i%3)===0?'minor issue being addressed ⚠️':'all systems green ✅'}</li></ul><p>Next sync: ${pick(MONTHS,i+1)} ${(i%28)+1}. Join via: ${pickLinks(i,1)}</p><p>Cheers! 🥂</p></body></html>`;
  if (t === 9) return `<html><body style="font-family:Calibri,sans-serif;font-size:14px"><p>STANDUP NOTES — ${pick(DEPTS,i).toUpperCase()} TEAM</p><hr style="border:none;border-top:1px solid #ccc"/><p><strong>${pick(SENDERS,i).name}:</strong> Completed ${pick(TOPICS,i)} review. Will start ${pick(TOPICS,i+1)} today. No blockers.</p><p><strong>${pick(SENDERS,i+1).name}:</strong> ${pick(SYSTEMS,i)} configuration in progress. ETA: EOD.</p><p><strong>${pick(SENDERS,i+2).name}:</strong> Code review done. No blockers.</p><p>Sprint board: ${pickLinks(i,1)}</p></body></html>`;
  if (t === 10) return `<html><body style="font-family:Calibri,sans-serif;font-size:14px"><p>Forwarding this for your awareness.</p><p><strong>Original from:</strong> ${pick(SENDERS,i+3).name} | <strong>Subject:</strong> ${subject}</p><hr style="border:none;border-top:1px solid #ccc;margin:8px 0"/><p>${pick(TOPICS,i)} results attached. Key highlight: ${(i%5)===0?'critical issue found — see highlighted row':'all metrics within acceptable range'}.</p><p>Please review and confirm by <strong>${pick(MONTHS,i)} ${(i%28)+1}</strong>.</p><p>Additional context: ${pickLinks(i)}</p></body></html>`;
  return `<html><body style="font-family:Calibri,sans-serif;font-size:14px"><p>🎉 Great news about <strong>${subject}</strong>!</p><p>The team has done an amazing job 💪 finishing this on time.</p><p>Highlights: 🚀 Performance up 23% | 📉 Errors down 91% | ⏱️ ${(i%30)+5}% faster than baseline</p><p>Kudos to: ${pick(SENDERS,i).name} 🌟, ${pick(SENDERS,i+1).name} 🌟, ${pick(SENDERS,i+2).name} 🌟</p><p>Next steps: 🔔 Schedule retrospective | 📋 Update runbook | 🔄 Plan next sprint</p><p>Team lunch 🍕 on ${pick(MONTHS,i)} ${(i%28)+1}! RSVP: ${pickLinks(i,1)}</p></body></html>`;
}

function plainBody(i, subject) {
  const t = i % 5;
  if (t === 0) return `Hi team,\n\nUpdate on: ${subject}\n\nStatus: ${['In Progress','Complete','Pending Review','Blocked','On Hold'][i%5]}\nOwner: ${pick(SENDERS,i).name}\nDeadline: ${pick(MONTHS,i)} ${(i%28)+1}, ${pick(YEARS,i)}\nArchive folder: ${PARENT_FOLDER}\n\nAction required: Please respond by EOD.\n\nRegards,\n${pick(SENDERS,i).name}`;
...
function getAttachments(i) {
  ...
  return undefined; // 12-19: no attachment (~60% of messages)
}

// ─── Message definition builder ───────────────────────────────────────────────
function buildMsgDef(globalIndex, opts = {}) {
  const i       = globalIndex;
  const subject = opts.subject || pick(SUBJECT_TEMPLATES, i)(i);
  const isHtml  = (i % 3) !== 2;
  const body    = isHtml
  ...
  return {
    ...
    toRecipients: pick(TO_PATTERNS, i),
    ccRecipients: pick(CC_PATTERNS, i).length > 0 ? pick(CC_PATTERNS, i) : undefined,
    bccRecipients: pick(BCC_PATTERNS, i),
    flag: flagStatus ? { flagStatus } : undefined,
    categories: cats,
    attachments: opts.attachments !== undefined ? opts.attachments : getAttachments(i),
...
// ─── Generate messages for a folder ───────────────────────────────────────────
function generateFolderMessages(count, baseIndex, folderName) {
  const messages = [];
  let idx = baseIndex;

  // ~15% thread chains: pick chain count from count
  const chainCount = Math.max(1, Math.round(count * 0.15 / 5.5)); // avg 5.5 msgs/chain
  const chainLengths = [5, 6, 7, 8, 9, 10, 5, 6, 7, 8];

  for (let c = 0; c < chainCount; c++) {
    const len      = chainLengths[c % chainLengths.length];
    const chainId  = `${folderName.replace(/[^a-z0-9]/gi,'')}-chain-${c+1}-${baseIndex}`;
    const ids      = Array.from({length: len}, (_, m) => `<${chainId}-m${m+1}@qatestagent.com>`);
    const rootSubj = pick(SUBJECT_TEMPLATES, idx)(idx);
    const chainMs  = OLD_START + (c / chainCount) * (OLD_END - OLD_START);

    for (let m = 0; m < len; m++) {
      const received = new Date(chainMs + m * 4 * 3600_000).toISOString().replace(/\.\d{3}Z$/, 'Z');
      const def = buildMsgDef(idx, {
        subject: m === 0 ? rootSubj : `RE: ${rootSubj}`,
  ...
  // Fill remaining with standalone old messages
  ...
  async function worker() {
    while (i < total) {
      ...
      try {
        await tasks[j]();
        ok++;
        if (ok % 100 === 0 || ok === total) {
          const elapsed = (Date.now() - startTime) / 1000;
          const rate    = ok / elapsed;
          const eta     = Math.round((total - ok) / rate);
          const pct     = Math.round(ok / total * 100);
          console.log(`  [${label}] [${ok}/${total}] ${pct}% | ${rate.toFixed(1)} msg/s | ETA ~${Math.floor(eta/60)}m ${eta%60}s | failures: ${fail}`);
        }
      } catch (err) {
        fail++;
        if (fail <= 30) console.error(`  ✗ [${label}][${j}] ${err.message.substring(0, 100)}`);
      }
    }
  }
...
async function discoverFolders() {
  ...
  const hdr   = () => outlookClient.getAccessToken(TARGET).then(t => ({ Authorization: `Bearer ${t}` }));

  // Find InPlace_Archive_Test at top level
  const topRes = await axios.get(
    `${GRAPH_BASE}/users/${uid}/mailFolders?$filter=displayName eq '${PARENT_FOLDER}'&$select=id,displayName`,
    { headers: await hdr() }
  );
  if (!topRes.data.value.length) throw new Error(`Folder '${PARENT_FOLDER}' not found for ${TARGET}`);
  const parent = topRes.data.value[0];
  console.log(`Found parent folder: ${parent.displayName} (${parent.id})`);

  // Recursively collect all child folders
  async function getChildren(folderId, depth) {
    const h = await hdr();
    const res = await axios.get(
      `${GRAPH_BASE}/users/${uid}/mailFolders/${folderId}/childFolders?$select=id,displayName&$top=50`,
      { headers: h }
    );
    const children = res.data.value || [];
    const result = [];
    for (const f of children) {
      result.push({ id: f.id, name: f.displayName, depth });
      const sub = await getChildren(f.id, depth + 1);
      result.push(...sub);
    }
    return result;
  }

  const subfolders = await getChildren(parent.id, 1);
  return subfolders;
}

// ─── Main ─────────────────────────────────────────────────────────────────────
async function main() {
  console.log(`\nDiscovering folders under '${PARENT_FOLDER}' for ${TARGET}…`);
  const folders = await discoverFolders();

  if (folders.length === 0) {
    throw new Error(`No subfolders found under '${PARENT_FOLDER}'. Create the folder structure first.`);
  }

  // Assign message counts
  const folderBatches = folders.map(f => ({
    ...f,
    count: f.name.toLowerCase() === 'l1' ? L1_COUNT
         : f.name.toLowerCase() === 'l5' ? L5_COUNT
         : OTHER_COUNT,
  }));

  const totalMessages = folderBatches.reduce((s, f) => s + f.count, 0);
  console.log(`\nFolder plan:`);
  folderBatches.forEach(f => console.log(`  ${f.name.padEnd(6)} (depth ${f.depth}): ${f.count.toLocaleString()} messages`));
  console.log(`\nTotal: ${totalMessages.toLocaleString()} messages across ${folders.length} folders`);
  console.log(`Concurrency: ${CONCURRENCY} | Estimated runtime: ${Math.round(totalMessages / (CONCURRENCY * 0.8 * 60))}–${Math.round(totalMessages / (CONCURRENCY * 0.4 * 60))} minutes\n`);

  let baseIndex = 0;
  let grandOk = 0, grandFail = 0;
  const overallStart = Date.now();

  for (const folder of folderBatches) {
    console.log(`\n── ${folder.name} (${folder.count.toLocaleString()} msgs) ──`);
    const messages = generateFolderMessages(folder.count, baseIndex, folder.name);
    baseIndex += folder.count;

    const tasks = messages.map((msg) => async () => {
      await outlookClient.createMessageInFolder(TARGET, folder.id, msg);
    });

    const { ok, fail } = await runPool(tasks, CONCURRENCY, folder.name);
    grandOk   += ok;
    grandFail += fail;
    console.log(`  ${folder.name} done: ${ok}/${folder.count} created, ${fail} failed`);
  }

  const elapsed = Math.round((Date.now() - overallStart) / 1000);
  console.log(`\n${'='.repeat(60)}`);
  console.log(`All folders complete in ${Math.floor(elapsed/60)}m ${elapsed%60}s`);
  console.log(`Created: ${grandOk}/${totalMessages} (${grandFail} failed)`);
  console.log(`Folders: ${folders.map(f=>f.name).join(', ')}`);
  console.log(`Thread chains: ~15% per folder (5–10 levels deep)`);
}
