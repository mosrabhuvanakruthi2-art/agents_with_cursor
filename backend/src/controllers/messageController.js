...
// Handlers for the message-migration vertical (Slack / Google Chat / Teams).
// Extracted from the Nagalakshmi branch and kept SEPARATE from the mail/content
// agentController. Uses the message-owned chatMigrationClient (not the mail client).
...
async function migrateMessageAgent(req, res) {
  try {
    ...
    // The selected channels & DMs migrate exactly once. We deliberately do NOT fan
    // out over mappedPairs (that would re-run the same channel migration once per
    // mapped user). Fall back to the first mapped pair only if explicit
    // source/destination workspace admins were not supplied.
    const srcWorkspace = sourceEmail || sharedOpts.sourceAdminEmail || mappedPairs?.[0]?.sourceEmail;
    const dstWorkspace = destinationEmail || mappedPairs?.[0]?.destinationEmail;
    if (!srcWorkspace || !dstWorkspace) {
      return res.status(400).json({ error: 'sourceEmail (source workspace admin) and destinationEmail (destination workspace admin) are required' });
    }

    const context = new MessageMigrationContext({
      ...sharedOpts,
      sourceEmail: srcWorkspace,
